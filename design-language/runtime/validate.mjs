...
import { adaptCarouselFormat } from './format-adapter.mjs';

const here = path.dirname(fileURLToPath(import.meta.url));
const outDir = path.join(here, '..', 'public', 'intelligence', 'v1', 'specimens', 'design-runtime');
fs.mkdirSync(outDir, { recursive: true });

function writeCarouselSpec(spec, outputPrefix) {
  ...
  fs.writeFileSync(path.join(outDir, `${outputPrefix}.manifest.json`), JSON.stringify({
    runtime:'GSIL.DESIGN_RUNTIME', runtime_version:'1.4.0', doctrine:'GSIL.DESIGN.THROUGH_FLOW_V4', specimen:spec.id, format:spec.format, slide_count:rendered.length,
...
function readSpecimen(file) {
  return JSON.parse(fs.readFileSync(path.join(here, 'specimens', file), 'utf8'));
}

const opener = readSpecimen('decision-frame-carousel.json');
...
const standard = writeCarouselSpec(readSpecimen('decision-frame-carousel-full.json'), 'decision-frame-carousel');
const autoSpec = readSpecimen('decision-frame-carousel-auto-components.json');
const automatic = writeCarouselSpec(autoSpec, 'decision-frame-carousel-auto');
const linkedInSpec = adaptCarouselFormat(autoSpec, 'linkedin_square');
const linkedIn = writeCarouselSpec(linkedInSpec, 'decision-frame-linkedin-square');

const componentSet = readSpecimen('content-components.json');
...
fs.writeFileSync(path.join(outDir, 'content-components.manifest.json'), JSON.stringify({ runtime:'GSIL.DESIGN_RUNTIME', runtime_version:'1.4.0', specimen:componentSet.id, component_count:componentManifest.length, components:componentManifest }, null, 2));
...
console.log(`GSIL LinkedIn Adaptation validated: ${linkedIn.spec.id} (${linkedIn.rendered.length} square slides)`);
